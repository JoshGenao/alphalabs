...
//!
//! The authority is owned by the [`ExecutionEngine`] (a private field reached
//! only through `designate` / `demote` / `route_order`); callers cannot supply a
//! forged `LiveDesignation`. [`ExecutionEngine::route_order`] derives live-ness
//! from that engine-owned authority, not a caller-supplied `StrategyMode`. A
//! strategy that is not the single designated live strategy is rejected with
//! `NON_LIVE_STRATEGY_SUBMISSION` **before any broker / connectivity / freshness
//! port is consulted** — proven with `Forbidden*` stubs that panic if touched
//! and a `ForbiddenBroker` that panics on any `submit_order`. The
...
fn srs_exe_001_only_the_designated_strategy_routes_to_the_broker() {
    ...
    assert_eq!(
        engine.designated().map(StrategyId::as_str),
        Some("live-alpha")
    );
}
...
fn srs_exe_001_non_designated_strategy_is_rejected_before_any_port() {
    // The engine designates live-alpha; a DIFFERENT strategy submitting an order
    // must be rejected synchronously with NON_LIVE_STRATEGY_SUBMISSION and must
    // not consult the broker, connectivity, or freshness ports (all are
...
fn srs_exe_001_exactly_one_designation_is_demotable_and_re_designable() {
    ...
    // through the engine-owned authority.
    let mut engine = ExecutionEngine::default();
    engine
        .designate(StrategyId::new("live-alpha"), confirm("live-alpha"))
