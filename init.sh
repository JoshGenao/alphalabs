...
echo "→ Running operator interface runtime check..."
if ! python3 tools/operator_interface_runtime_check.py >/dev/null; then
  echo "✗ Environment failed"
  echo "  Operator interface runtime check failed; run python3 tools/operator_interface_runtime_check.py for detail."
  exit 1
fi
...
echo "→ Running order-lifecycle contract check..."
if ! python3 tools/order_lifecycle_check.py >/dev/null; then
  echo "✗ Environment failed"
  echo "  Order-lifecycle contract check failed; run python3 tools/order_lifecycle_check.py for detail."
  exit 1
fi

echo "→ Running order-event dispatch contract check..."
if ! python3 tools/order_event_dispatch_check.py >/dev/null; then
  echo "✗ Environment failed"
  echo "  Order-event dispatch contract check failed; run python3 tools/order_event_dispatch_check.py for detail."
  exit 1
fi
...
echo "→ Running market-data idempotent-ingestion contract check (SRS-DATA-016)..."
if ! python3 tools/ingestion_idempotency_check.py --require-cargo >/dev/null; then
  echo "✗ Environment failed"
  echo "  Idempotent-ingestion contract check failed; run python3 tools/ingestion_idempotency_check.py --require-cargo for detail."
  exit 1
fi

echo "→ Running SSD/NAS tiered-storage contract check (SRS-DATA-008)..."
if ! python3 tools/data008_tiering_check.py --require-cargo >/dev/null; then
  echo "✗ Environment failed"
  echo "  Tiered-storage contract check failed; run python3 tools/data008_tiering_check.py --require-cargo for detail."
  exit 1
fi
