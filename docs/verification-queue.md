...
| **SRS-MD-005** | 1 | All four AC clauses ran solo and are recorded as executed with exit 0 (suspension, suppression, reconnection, post-window escalation), proven by fault injection against a genuinely dead loopback port. **The judgment critic verdict is `block` at 24 rounds**, so `evidence.py verify` refuses and `close_feature.py` exits 3. `--attested-by` relaxes which STEPS count, not the critic gate. There is exactly ONE route: run review rounds until the judgment layer returns APPROVE. Closing over a standing `block` has no mechanism - `close_feature.py` exposes no override for it, and hand-stamping `approve` would be faking a green, which CLAUDE.md rule 4 forbids. | 1. `python3 tools/adversarial_review.py <pre-feature-base>` and fix what it finds; repeat until APPROVE. 2. `evidence.py critic SRS-MD-005 --layer judgment --verdict approve --reviewer <who> --rounds <N>` (omit `--rounds`: the count is read from `review.jsonl`, which the approving round has just appended to; the previous stamp is used only if that ledger is missing or unreadable). 3. `close_feature.py SRS-MD-005 --verified --attested-by operator` **from the primary checkout**. |
