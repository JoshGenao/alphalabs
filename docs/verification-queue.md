...
| **SRS-MD-005** | 1 | All four AC clauses ran solo and are recorded as executed with exit 0 (suspension, suppression, reconnection, post-window escalation), proven by fault injection against a genuinely dead loopback port. **But the judgment critic verdict is `block` at 15 rounds**, so `evidence.verify` refuses and `close_feature.py` exits 3 - `--attested-by` relaxes which STEPS count, not the critic gate. Two routes: run further review rounds until APPROVE, or make an explicit operator decision to close over a standing block, as this row and `progress.d/session-SRS-MD-005.md` record. | `python3 tools/adversarial_review.py <pre-feature-base>` until APPROVE, then `evidence.py critic SRS-MD-005 --layer judgment --verdict approve`, then `close_feature.py SRS-MD-005 --verified --attested-by operator` **from the primary checkout** |
