...
| SN-1.06 | The trader needs to run a strategy in 'live' mode, selecting either a real brokerage account or the IB paper trading account for execution. | Paper trading validates system integration without capital risk; switching to a live account must be explicit and deliberate. | High |
...
| SN-1.10 | The trader needs to run up to 60 strategies concurrently (30 live + 30 paper), sized for the reference hardware (Intel i5-12400, 32 GB RAM). | The trader's research pipeline may produce many candidate strategies that must run in parallel for evaluation and diversification. The 60-container target (reduced from the original 50/100 range) reflects hardware resource constraints validated during SyRS analysis. | Medium |
| SN-1.11 | The trader needs a kill switch that immediately cancels all resting orders, liquidates all open positions, and disconnects from the exchange, accessible from the dashboard, CLI, and REST API. | In an emergency (runaway strategy, flash crash, connectivity degradation), the trader must be able to halt all activity instantly from any available interface. The dashboard is accessible via mobile browser; the REST API is callable from any HTTP client. | High |
...
| SN-1.25 | The platform shall maintain a Strategy Reservoir of at least 30 paper-trading strategies, providing a monthly ranking based on risk-adjusted returns (e.g., Sharpe ratio, Sortino ratio). The system must support a Hot-Swap capability to promote top-performing paper strategies to the live portfolio and demote live strategies that breach predefined drawdown thresholds, with configurable promotion and demotion criteria. Demotion shall liquidate all open positions before transitioning the strategy to paper mode. Promotion shall begin with no open positions (flat start). A configurable cool-down period (default: 7 days) shall prevent promotion/demotion oscillation. | A reservoir of paper-trading strategies creates a continuous pipeline for strategy evaluation and selection under real market conditions without capital risk. Automated promotion/demotion based on risk-adjusted performance reduces the lag between strategy validation and deployment, while drawdown-based demotion provides a systematic risk control mechanism that does not rely on manual intervention by the solo operator. The 30-strategy reservoir target (reduced from 50) reflects hardware resource constraints. | High |
...
| SN-2.04 | The operator needs connectivity fail-safes that automatically detect when the IB API is unreachable and prevent order submission until connectivity is restored. | Submitting orders during an API outage can result in unacknowledged or duplicate orders. | High |
...
| C-2 | The initial brokerage integration shall be Interactive Brokers TWS API (https://www.interactivebrokers.com/campus/ibkr-api-page/twsapi-doc/). | Stakeholder | Execution layer must implement IB TWS API protocol; paper trading uses IB's built-in paper account |
...
| A-2 | The IB paper trading account provides a sufficiently realistic simulation environment for validating strategy execution logic before going live. | If paper trading behavior diverges significantly from live (e.g., unrealistic fills), an internal simulation engine may be required. |
| A-3 | A single Proxmox VM running a Docker Compose stack on the reference hardware (i5-12400, 32 GB RAM) can support 60 concurrent strategy containers (30 live + 30 paper) with acceptable performance, given a workload priority hierarchy and 2 GB memory safety margin. | If resource requirements exceed the reference hardware, either the container target must be reduced or a hardware upgrade / multi-node deployment is needed. |
...
| A-9 | Corporate action data (splits, dividends, delistings, mergers, symbol changes) is available through IB's API or a supplemental data source with sufficient history and timeliness for both backtesting and live trading. | If corporate action data is incomplete or delayed, backtests will contain price discontinuities and live strategies may trade on unadjusted data, producing incorrect position sizing. |
...
- [ ] **SC-4:** The web-based dashboard displays live performance metrics, benchmark comparison, heartbeat status, strategy logs, and system health with ≤5-second refresh latency. *(Traces to: SN-2.01, SN-2.02, SN-2.03)*
- [ ] **SC-5:** The kill switch cancels all resting orders, submits liquidation orders for all positions within 5 seconds of activation, and disconnects from the exchange (with a 30-second timeout for unfilled liquidation orders). The kill switch is accessible from dashboard, CLI, and REST API. *(Traces to: SN-1.11)*
...
- [ ] **SC-7:** 60 strategies (30 live + 30 paper) run concurrently without degradation of order execution latency or dashboard responsiveness. *(Traces to: SN-1.10)*
...
- [ ] **SC-11:** The system automatically adjusts historical prices, open order quantities, and portfolio positions for stock splits, reverse splits, and dividends; backtests spanning corporate action dates produce correct P&L calculations. *(Traces to: SN-1.14, SN-1.15)*
...
- [ ] **SC-17:** Order event callbacks deliver fill price, fill quantity, and commission to user strategy code within 1 second of fill acknowledgement from the broker. *(Traces to: SN-1.22)*
...
- [ ] **SC-20:** The Strategy Reservoir maintains at least 30 concurrent paper-trading strategies, produces a monthly risk-adjusted ranking, and successfully promotes a top-ranked paper strategy to live (flat start, no open positions) and demotes a live strategy that breaches a configured drawdown threshold (positions liquidated before demotion) without manual intervention. A 7-day cool-down prevents oscillation. *(Traces to: SN-1.25)*
...
- **OS-6:** Internal paper-trading simulation engine. IB's built-in paper trading account satisfies paper trading needs for this phase.
...
| SN-1.06 | Live or paper trading mode selection | BG-1 |
...
| SN-1.10 | 60 concurrent strategies (30 live + 30 paper) | BG-1, BG-6 |
| SN-1.11 | Kill switch (dashboard, CLI, REST API) | BG-1, BG-4 |
...
| SN-1.25 | Strategy Reservoir (30 paper strategies, monthly ranking, Hot-Swap with liquidate-before-demotion) | BG-1, BG-2, BG-4 |
...
| SN-2.04 | Connectivity fail-safes | BG-1, BG-4 |
