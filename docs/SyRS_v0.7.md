...
**Document ID:** SyRS
**Version:** 0.7  
**Status:** Complete for SRS derivation  
**Last updated:** 2026-04-10  
**Traces from:** StRS v0.7
...
deploy Python-based algorithmic trading strategies for US equities and
options through Interactive Brokers.

The system supports one live strategy executing against the Interactive
Brokers live account at a time. Additional concurrently running
strategies execute in internal paper-trading simulation using shared live
market data and independent simulated positions and P&L tracking.

The ATP provides capabilities for strategy execution, backtesting,
historical and live market data access, factor computation, research
support, operational monitoring, notification, and controlled promotion
of internally simulated strategies to the live trading slot.

Reference deployment assumptions and implementation-specific environment
details are documented elsewhere in this specification and supporting
...
| IF-11 | Push Notification | Outbound | ntfy publish API | SN-1.12 | Connectivity loss, critical failure alerts |
| IF-12 | Archival Storage Interface | Bidirectional | File-based archival storage interface | C-5, SN-1.26, SN-1.27 | Archival storage tier for persistent market and reference data; implementation details are defined in deployment documentation |
...
| IF-15 | Bulk Equity Data Provider (Databento, Phase 1) | Inbound | Databento API / file download (DBN, CSV, or Parquet), abstracted by SYS-56 interface | SN-1.26, SN-3.03, BG-3 | Provides daily OHLCV for the full US equity universe and minute-bar historical backfill. Accessed via the vendor-agnostic data provider interface (SYS-53, SYS-56). |

---

## 4. Functional requirements (system-level)

This section defines the system-level functional requirements for ATP.
Each requirement traces to one or more stakeholder needs, business goals,
or constraints in StRS-001. Requirements are written using "shall"
statements and are intended to be clear, testable, and traceable. Where
a stakeholder-imposed architectural or operational constraint affects
implementation, that constraint is stated explicitly.
...
| SYS-12 | The Strategy Orchestrator shall provide an internal service interface through which strategy containers access the Data Layer, Execution Engine (live) or Internal Simulation Engine (paper), and logging infrastructure without direct inter-container coupling. The interface mechanism shall be specified in the supporting architecture documentation. | SN-3.01, BG-5 | P1 |
...
| SYS-15b | The backtesting engine shall model slippage using a configurable slippage model. The default slippage model shall be 0.05% of trade notional per trade. | BG-2, SN-1.03 | P1 |
| SYS-15c | The backtesting engine shall model bid-ask spread impact using a configurable spread-impact model. The default spread-impact model shall use the observed bid-ask spread when available and shall apply a fallback spread impact of 0.10% of trade notional when spread data is unavailable. | BG-2, SN-1.03 | P1 |
...
| SYS-23 | The system shall capture option chain data from IB for all currently live (non-expired) contracts during a configurable near-close capture window on each trading day, with the capture window ending no later than the regular market close. The capture shall include at minimum: underlying, expiration, strike, right (call/put), bid, ask, last, volume, open interest, and implied volatility. | BG-2, BG-7, SN-1.27, C-7 | P1 |
...
| SYS-31 | Nightly IB minute-bar watchlist ingestion (SYS-22b) shall complete within the overnight window (16:00 ET to 09:30 ET next trading day). IB option chain capture (SYS-23) shall complete within the configured near-close capture window on each trading day. Both IB activities shall respect IB pacing limits (no more than 60 historical data requests per 10-minute period, no identical requests within 15 seconds). Nightly bulk vendor ingestion (SYS-22a daily bars) shall complete within the overnight window. | BG-3, SN-1.26, SN-1.27, A-10 | P1 |
...
| SYS-43 | The dashboard shall provide direct navigation to the embedded Jupyter research environment from the primary operator workflow. | BG-4, BG-7, SN-1.18, SN-2.01 | P2 |
...
| SYS-46 | The system shall notify the user through email and push notification within 60 seconds of detecting connectivity loss to IB Gateway or any critical system failure. | BG-4, SN-1.12 | P1 |
...
| SYS-48 | The system shall rank Strategy Reservoir strategies over a configurable shared evaluation window selectable from 1, 7, 15, 30, 60, or 90 calendar days, defaulting to 30 calendar days, using risk-adjusted return metrics computed from internal simulation performance. The ranking shall support at minimum Sharpe ratio and Sortino ratio. The system shall also compute a momentum score defined as the rate of change of the selected risk-adjusted return metric over the same evaluation window. Ranking outputs, including momentum score, shall be displayed on the dashboard and exposed through the REST API. | BG-1, BG-2, BG-4, SN-1.25, SN-1.30 | P1 |
...
| ID | Requirement | Traces to (StRS) | Priority |
|----|------------|-------------------|---------|
| SYS-55 | The system shall validate scheduled IB historical data request volume against the applicable IB pacing limits for each capture job window. For SYS-22b, the projected request count for the overnight minute-bar watchlist ingestion window shall not exceed the requests permitted during that window. For SYS-23, the projected request count for the configured near-close capture window shall not exceed the requests permitted during that window. If either projected request count exceeds the applicable limit, the system shall alert the operator at scheduling time and shall refuse to start the affected ingestion job until scope or window configuration is reduced. | BG-3, SN-1.26, SN-1.27, A-10 | P1 |
...
| ID | Requirement | Traces to (StRS) | Priority |
|----|------------|-------------------|---------|
| SYS-70 | The system shall maintain a centralized market data subscription manager that consolidates real-time market data requests from all active strategy containers (live and paper) into a deduplicated set of IB market data subscriptions. When multiple strategy containers subscribe to the same security, the system shall maintain a single IB market data subscription and distribute received data to all subscribing containers. The subscription manager shall enforce the operator-configured IB account concurrent market data line limit, where the configured value shall be verified during deployment or commissioning against the active IB account and subscription package. When a new subscription request would exceed the configured limit, the system shall return a structured error to the requesting strategy (SYS-64) and alert the operator on the dashboard. | SN-1.10, SN-1.07, SN-1.29, C-7, A-13, BG-1 | P1 |
...
| NFR-P6 | Connectivity loss notification delivery | ≤ 60,000 ms | From detection to notification dispatch via email and push notification | SN-1.12, SN-2.04 |
...
| NFR-P8c | IB option chain capture completion | SYS-23 shall complete within the configured near-close capture window on each trading day while respecting IB pacing limits per SYS-55. | Option chains | SN-1.27, A-10 |
...
| NFR-P10 | The system shall meet NFR-P1 (order latency < 1,000 ms p95) and NFR-P2 (dashboard refresh ≤ 5,000 ms) simultaneously when the baseline active strategy set defined by NFR-SC1 is processing market data events. | Baseline active strategy set defined by NFR-SC1 | BG-1, SN-1.10 |
...
| NFR-S4 | Notification channel credentials (SMTP, push service API tokens) shall be stored encrypted at rest and shall not appear in logs. | SN-1.12 | OWASP credential storage guidelines |
...
| NFR-R6 | A full system restart during market hours — including Proxmox VM availability, OS boot, Docker daemon startup, ATP service initialization, and completion of the readiness check in SYS-76 — shall restore the platform to a trade-ready state within 10 minutes. | ≤ 10 minutes to trade-ready state | SN-2.05 |
...
| NFR-SC2 | The data layer shall accommodate the full US equity universe (8,000+ securities) at daily and minute-bar resolution, plus option chain snapshots and Sharadar fundamental data, using tiered storage: a local 1 TB SSD as the primary runtime storage tier and the 20 TB NAS as the archival tier. Storage growth estimates shall be documented in SRS. | SN-2.06, SN-1.26, SN-1.27, C-5 | SSD retention window: 90 days (configurable). NAS I/O validated by benchmark: sequential read 112 MB/s (adequate for cold reads), concurrent random I/O insufficient for runtime use (513 ms latency at 60 workers). |
...
This section defines stakeholder-imposed and architecture-governing
constraints that limit the allowable solution space for ATP
implementation. These constraints shall be considered binding unless
superseded by an approved change to this specification. Compliance may be
assessed through review, analysis, inspection, and automated checks where
applicable.
...
| AC-5 | The NAS (20 TB) shall be the archival storage tier for all ingested market data and fundamental data. The local SSD (1 TB) shall be the primary runtime storage tier. | C-5 | Deployment configuration; data path audit |
...
| AC-16 | Core ATP runtime services — including the strategy orchestrator, execution engine, internal simulation engine, market data subscription manager, data ingestion and storage services, factor pipeline runtime services, and notification dispatcher — shall be implemented in Rust. User-authored strategies shall remain Python per AC-1. The dashboard backend/API and presentation layer may be implemented in another language. | C-12 | Architecture review; build manifest audit |
...
| Notification Event | Event type, timestamp, channels dispatched (email, push), delivery status | Notification subsystem | SN-1.12 |
...
This section documents the reference deployment environment used to size
performance, storage, and operational requirements. Unless explicitly
referenced by a requirement, non-functional requirement, or
architectural constraint, the contents of this section are informative
and do not themselves impose additional system requirements.
...
| CI/CD | GitHub Actions (free tier) | Stakeholder decision |
| IB connectivity | IB Gateway (headless), ports 4001 (live) / 4002 (paper/integration testing) | C-2, stakeholder decision |
| Primary storage (SSD) | Local 1 TB SSD on Proxmox host, ext4 or XFS. Runtime storage for all active data: recent bars, strategy state (live and paper), backtest working sets, factor pipeline inputs, logs. | C-5 |
| Archival storage (NAS) | TrueNAS, 20 TB, HDD-based, 4 GB ZFS ARC, NFS v3, direct-attached (10.0.0.20). Archival tier for full historical data. Sequential read: 112 MB/s. Concurrent random I/O inadequate for runtime use (validated by benchmark). | C-5 |
| Storage architecture | Tiered: SSD primary (runtime read/write) + NAS archival (sync + cold reads). Ingestion writes to SSD first, syncs to NAS post-ingestion. | Architecture decision |
| Fundamental data | Sharadar (Phase 1) | Stakeholder decision |
| Bulk equity data | Databento (Phase 1 vendor). | Option D architecture decision |
...
| Recurring infrastructure cost target | Near-zero (on-premise compute and storage; external costs limited to IB subscriptions, Sharadar subscription, bulk equity vendor subscription, email/push service) | BG-6 |
| Concurrency target | 1 live + at least 30 paper strategy containers on reference hardware baseline; higher concurrency requires validation | SN-1.10, C-11 |
...
| SYS-2a | SN-1.06 | BG-1 |
| SYS-2b | SN-1.06, SN-1.29 | BG-1 |
| SYS-2c | SN-1.06, SN-1.11 | BG-1 |
| SYS-2d | SN-1.06 | BG-1 |
| SYS-2e | SN-1.06, SN-1.29 | BG-1 |
| SYS-3 | SN-1.08 | BG-1 |
| SYS-4 | SN-1.24 | BG-1 |
...
| SYS-15a | SN-1.03 | BG-2 |
| SYS-15b | SN-1.03 | BG-2 |
| SYS-15c | SN-1.03 | BG-2 |
| SYS-15d | SN-1.03 | BG-2 |
| SYS-15e | SN-1.03, SN-1.29 | BG-2 |
...
| SYS-31 | SN-1.26, SN-1.27, A-10 | BG-3 |
| SYS-32 | SN-2.06 | BG-3 |
| SYS-33 | SN-2.06 | BG-3 |
...
| SYS-43 | SN-1.18, SN-2.01 | BG-4, BG-7 |
...
| SYS-54 | SN-3.04 | BG-5 |
| SYS-55 | SN-1.26, SN-1.27, A-10 | BG-3 |
...
All previously open items from v0.6 have been resolved and incorporated
into the baseline captured in this document.

| Prior open item | Resolution incorporated in v0.7 |
|-----------------|----------------------------------|
| CI/CD platform selection | GitHub Actions free tier selected for automated checks and architectural-constraint enforcement support. |
| Option chain capture timing | Near-close option-chain capture selected. SYS-23, SYS-31, SYS-55, and NFR-P8c updated accordingly. |
| Full-system market-hours restart RTO | Trade-ready restart target set to 10 minutes in NFR-R6. |
| Bulk equity data provider selection | Databento selected as the Phase 1 bulk equity data provider. IF-15 and Section 8 updated accordingly. |
| IB market data line ceiling | The exact numeric ceiling is treated as an operator-configured account parameter verified during deployment or commissioning against the active IB account and subscription package. SYS-70 updated accordingly. |
| Reservoir evaluation window selection | Shared evaluation window retained, selectable from 1, 7, 15, 30, 60, or 90 calendar days, defaulting to 30. SYS-48 updated accordingly. |
| Default slippage and spread-impact baseline | Approved as 0.05% slippage per trade and 0.10% fallback spread impact when spread data is unavailable. SYS-15b, SYS-15c, and SYS-83 use this baseline. |
| Backend implementation language | Core ATP runtime services constrained to Rust; user strategies remain Python; dashboard backend/API may use another language. AC-16 added. |
...
|---------|---------------|------------|------------|-----------|
| SYS-9, SYS-47, NFR-SC1 | SN-1.10, SN-1.25 | 1 live + up to 59 paper (60 total) | 1 live + 30 paper (release baseline) | The reference hardware baseline and initial verification scope support 30 paper strategies for release. Higher concurrency remains an intended expansion target subject to performance validation. |
...
| 0.6 | 2026-04-07 | ChatGPT (edited draft) | Cleaned the SyRS for baseline readability and internal consistency. Rewrote Sections 1 and 2 for clearer SyRS scope; generalized IF-12 to remove deployment-specific interface detail; revised the Section 4 introduction; decomposed SYS-2 into SYS-2a through SYS-2e; changed SYS-9 and NFR-SC1 to a 30-paper-strategy release baseline with validated expansion beyond that baseline; decomposed SYS-15 into SYS-15a through SYS-15e; clarified SYS-43 so it no longer duplicates SYS-34a; aligned SYS-48 to the StRS momentum definition; revised NFR-S3 to remove unresolved reverse-proxy ambiguity; replaced agent-oriented language in Section 6; refined AC-9 to separate user-defined trading risk logic from platform-enforced safeguards; added an informative intro to Section 8; updated traceability rows to match the revised requirement IDs; removed resolved TBDs related to NFR-S3 and momentum-definition ambiguity; and updated the deviation note to reflect a release baseline rather than a contradiction. |
| 0.7 | 2026-04-10 | ChatGPT (stakeholder-closure update) | Closed the remaining open items needed for SRS handoff. Selected Databento as the Phase 1 bulk equity data provider and updated IF-15 and deployment assumptions. Changed option-chain capture from nightly batch to a configurable near-close capture window and updated SYS-23, SYS-31, SYS-55, and NFR-P8c. Added NFR-R6 to set a 10-minute full-system market-hours restart RTO. Revised SYS-48 to use a shared evaluation window selectable from 1, 7, 15, 30, 60, or 90 calendar days. Approved the default transaction-cost baseline of 0.05% slippage per trade and 0.10% fallback spread impact. Added AC-16 to constrain core runtime services to Rust while retaining Python for user-authored strategies and allowing the dashboard backend/API to use another language. Resolved the market-data line-limit open item by treating the limit as an operator-configured account parameter validated during deployment or commissioning. Added GitHub Actions free tier as the selected CI/CD platform. |
| 0.7.1 | 2026-08-17 | Claude (SRS-NOTIF-001) | **IF-11 changed from SMS to push notification (ntfy).** US A2P 10DLC registration is weeks of lead time and carriers filter unregistered traffic silently, so the SMS channel could not be proven to deliver; a self-hosted ntfy on the LAN, reached from the operator's phone over VPN, preserves SN-1.12's intent without it. Updated IF-11, SYS-44b, SYS-46, SYS-49c, NFR-P6, NFR-S4, the context diagram, the Notification Event data-dictionary row and the cost model. SC-9 unchanged. |
