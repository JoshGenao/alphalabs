...
**Document ID:** StRS
**Version:** 0.7  
**Status:** Updated for SyRS traceability closure  
...
| SN-1.12 | The trader needs to be notified through email, push notification, and dashboard alert when the system loses internet connectivity or critical failures occur. Future phases may extend notification to additional channels (SMS, Telegram, Discord). | As a solo operator, the trader cannot monitor the system continuously; multi-channel notification ensures awareness of problems regardless of the trader's current context. Email and push notification provide sufficient coverage for Phase 1. | High |
...
| C-10 | Daily OHLCV equity data for the full US universe and bulk historical minute-bar backfill shall be sourced from Databento. Minute-bar data for the watchlist and live option chain data shall continue to be sourced from IB. | Stakeholder | Data layer must implement the Databento adapter through the vendor-agnostic bulk equity provider interface; IB pacing budget remains reserved for watchlist minute bars and option-chain capture |
...
| C-12 | Core ATP runtime services — including orchestration, order routing, internal simulation, market-data subscription management, data ingestion/storage services, factor-pipeline runtime services, and notification dispatch — shall be implemented in Rust. User-authored strategies shall remain Python. The dashboard backend/API and presentation layer may use another language. | Stakeholder | Architecture and build system must support a mixed-language solution in which Python is limited to user strategies and non-core dashboard/backend components as approved by the stakeholder. |
...
- [ ] **SC-22:** The system captures and stores option chain data (bid, ask, last, volume, OI, IV at minimum) for live contracts during the configured near-close capture window on each trading day; data for a contract that subsequently expires remains available in the historical store indefinitely (NAS archival tier). *(Traces to: SN-1.27)*
...
| SN-1.12 | Connectivity notifications (email, push, dashboard alert — Phase 1) | BG-4 |
...
| SN-1.25 | Strategy Reservoir (30 internal-sim paper strategies, shared evaluation window selectable from 1/7/15/30/60/90 days, Hot-Swap swap model with 3 promotion modes) | BG-1, BG-2, BG-4 |
...
| SN-1.27 | Near-close forward-capture of live option chain data from IB before expiration; indefinite retention | BG-2, BG-7 |
...
| 0.7 | 2026-04-10 | ChatGPT (stakeholder-closure update) | Updated the StRS to close the traceability gaps needed to finalize the SyRS. Added C-12 to require Rust for core ATP runtime services while keeping Python for user-authored strategies and permitting another language for the dashboard backend/API. Selected Databento as the bulk equity data provider in C-10. Revised SN-1.27 and SC-22 to use a near-close option-chain capture window. Refined SN-1.25 to use a shared evaluation window selectable from 1, 7, 15, 30, 60, or 90 calendar days. Updated the traceability summaries accordingly. |
| 0.7.1 | 2026-08-17 | Claude (SRS-NOTIF-001) | **SN-1.12 Phase 1 channels changed from email/SMS to email/push notification**; SMS moves to the future-phases list beside Telegram and Discord. US A2P 10DLC registration is weeks of lead time with silent carrier filtering, so SMS could not be shown to reach the trader; a self-hosted ntfy reached over VPN preserves the need this requirement states — awareness regardless of the trader's current context. Rationale sentence otherwise unchanged; SC-9 unchanged. |
