...
class BacktestScriptTest(unittest.TestCase):
    def test_srs_bt_001_contract_script_passes(self) -> None:
        ...
        for needle in (
            ...
            "BacktestError with 10 fail-closed variants (EmptySymbol, InvalidDateRange, UnexpectedSymbol, NonPositivePrice, DuplicateBar, EmptyData, StrategyFailed, Overflow, TooManyBars, DataSourceMismatch)",
            "port trait BarSource with 2 method (source, bars); bars takes a max_bars read bound",
            "deferred Parquet / system-catalog reader seam",
