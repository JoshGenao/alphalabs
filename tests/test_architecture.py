...
class ArchitectureBoundaryTest(unittest.TestCase):
    def test_srs_arch_001_language_boundary(self) -> None:
        ...
        # ERR-5 / SRS-DATA-013 / SyRS SYS-77 ingestion-validation gate.
        self.assertIn(
            "atp-data gates ingestion writes on RecordValidationOutcome",
            result.stdout,
        )
        self.assertIn("2 states / 6 quarantine reasons", result.stdout)
        self.assertIn(
            "publishing IngestionValidationEvent when a record fails",
            result.stdout,
        )
    ...
    def test_srs_arch_005_configuration_pass(self) -> None:
        ...
        self.assertIn("17 keys catalogued across 6 categories", result.stdout)
        for category in (
            "credentials",
            "storage_paths",
            "ib_account",
