...
def assert_store_history(config: dict) -> list[str]:
    ...
    summary = (
        ...
        "fails closed rather than hanging. SRS-DATA-007 STAYS passes:false (foundational): a real strategy "
        "stand-in reads end-to-end through this binding (tests/domain/test_store_history_consumer.py), "
        "serving the gated split-adjusted Protocol default, but the acceptance NAMES backtests / factor "
        "jobs / notebooks as consumers and those engines are not yet WIRED to this store path (deferred to "
        "SRS-DATA-007); FULLY_ADJUSTED / TOTAL_RETURN remain deferred (dividend data, SRS-DATA-012). The "
