...
def assert_store_history(config: dict) -> list[str]:
    ...
    summary = (
        ...
        "no origin field read), serving RAW verbatim and the gated SPLIT_ADJUSTED (the Protocol default) "
        "through the SRS-DATA-011 coverage gate -- validating the echoed coverage_through frontier "
        "(gate-integrity) and raising CoverageNotProvenError (naming SRS-DATA-011) when uncovered, never "
        "raw-as-adjusted -- scaling OHLC by the named _PRICE_MINOR_SCALE while leaving volume a raw count, "
        "and invoking the CLI with a list argv (shell=False) under a bounded timeout so a wedged read "
        "fails closed rather than hanging. SRS-DATA-007 STAYS passes:false (foundational): the BACKTEST consumer "
        "is genuinely wired (atp-simulation store_bar_source::StoreBarSource consumes the unified store in "
        "BacktestEngine::run) and strategy + notebook/research code read via this binding "
        "(tests/domain/test_store_history_consumer.py); deferred -- the factor-job EXECUTION path (run_factor_job "
        "still takes caller-supplied inputs; the atp-factor-pipeline store_inputs loader is shipped substrate, needs "
        "Sharadar fundamentals SRS-DATA-005) and the Jupyter notebook HOST runtime (SRS-RES-002); "
...
def assert_normalization_modes(config: dict) -> list[str]:
    ...
    summary = (
        ...
        "and raises CoverageNotProvenError when uncovered -- the SRS-DATA-007 binding advance, though "
        "DATA-007 STAYS passes:false until the named consumers are wired). SRS-DATA-012 STAYS "
