...
def assert_store_history(config: dict) -> list[str]:
    ...
    summary = (
        ...
        "fails closed rather than hanging. SRS-DATA-007 is COMPLETE and closes to passes:true at integration (close_feature.py --verified flips feature_list.json under the scheduler lock; this branch deliberately does NOT edit that file) -- its AC is a contract test that all four named consumers query the interface provider-neutrally: the BACKTEST consumer "
